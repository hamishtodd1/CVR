...
THREE.VREffect = function ( renderer, onError ) {
	...
	this.toggleEyeSeparation = function()
	{
		...
		{
			eyesAreSeparated = 1;
			camera.near = 0.36; //55cm, tweaked
			camera.updateProjectionMatrix();
		}
		else if(eyesAreSeparated === 1)
		{
			eyesAreSeparated = 0;
			camera.near = 0.03; //1cm
			camera.updateProjectionMatrix();
		}
	}
	...
	this.render = function ( scene, camera, renderTarget, forceClear ) {
		...
		if ( vrDisplay && scope.isPresenting ) {
			...
			cameraL.position.lerp(centralPosition,1-eyesAreSeparated); 
			cameraR.position.lerp(centralPosition,1-eyesAreSeparated);
	...
	this.setPositionCorrection = function(newSetting)
	{
		positionCorrection.copy(newSetting)
	}
	...
	function getEyeMatrices( frameData ) {
		...
		if ( frameData.pose.position ) {
			...
			posePosition.add(positionCorrection)
