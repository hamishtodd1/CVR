...
THREE.VREffect = function ( eyesAreSeparated, renderer, onError ) {
	...
	{
		if(eyesAreSeparated === 0) //could have higher stereoscopy levels
		{
			eyesAreSeparated = 1;
		}
		else if(eyesAreSeparated === 1)
		{
			eyesAreSeparated = 0;
		}
	}
	...
	this.render = function ( scene, camera, renderTarget, forceClear ) {
		...
		if ( vrDisplay && scope.isPresenting ) {
			...
			//--------------OUR CODE
			var centralPosition = new THREE.Vector3().addVectors( cameraL.position, cameraR.position ).multiplyScalar(0.5);
			var amountOfTheirIpdWeAccept = 0.5; //Tweaked. They have slightly less than the actual amount, we want much less
			var lerpAmount = eyesAreSeparated ? 1-amountOfTheirIpdWeAccept : 1;
			cameraL.position.lerp(centralPosition, lerpAmount);
			cameraR.position.lerp(centralPosition, lerpAmount);
