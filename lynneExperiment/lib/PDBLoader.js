...
THREE.PDBLoader.prototype = {
	...
	parsePDB: function ( text ) {
		...
		for ( var i = 0, il = lines.length; i < il; ++ i ) {
			...
			//we're just taking in the first one for now TODO
			if (lines[ i ].substr( 0, 3 ) == "TER" )
				break;

			if (lines[ i ].substr( 0, 4 ) == "ATOM" 
//			 || lines[ i ].substr( 0, 6 ) == "HETATM" //Fuck 'em, temporarily
				) {
				...
				id =trim( lines[ i ].substr( 13, 4 ) ).toLowerCase();

				if ( e == "" ) e = trim( lines[ i ].substr( 12, 2 ) ).toLowerCase();
				atoms.push( [ x, y, z, CPK[ e ], capitalize( e ),id ] );
	...
	createModel: function ( json, callback ) {
		...
		for ( var i = 0; i < atoms.length; i ++ ) {
			...
			geometryAtoms.atomIDs.push( atom[ 5 ] );

		}

		if(bonds.length)
			console.log("we have some bonds")
